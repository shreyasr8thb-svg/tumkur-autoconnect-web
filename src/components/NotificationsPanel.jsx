...
export default function NotificationsPanel({ onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', zIndex: 9999, display: 'flex', justifyContent: 'flex-end', animation: 'fadeIn 0.2s' }}>
      <div className="glass-card flex-col" style={{ width: '100%', maxWidth: '380px', height: '100%', background: 'rgba(15,23,42,0.98)', borderRadius: 0, animation: 'slideInRight 0.3s forwards', borderLeft: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <div className="flex justify-between items-center p-4 border-b-dark" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
          <div className="flex items-center gap-2">
            ...
            {unreadCount > 0 && (
              <span style={{ background: '#e11d48', color: '#fff', fontSize: '0.7rem', padding: '2px 8px', borderRadius: '10px', fontWeight: 700 }}>
                {unreadCount} New
              </span>
            )}
          </div>
          <X size={24} color="#94a3b8" onClick={onClose} style={{ cursor: 'pointer' }} />
        </div>
        ...
        {notifications.length > 0 && (
          <div className="flex justify-between items-center px-4 py-2 border-b-dark" style={{ borderColor: 'rgba(255,255,255,0.02)', background: 'rgba(0,0,0,0.2)' }}>
            ...
            <button onClick={clearAll} style={{ background: 'none', border: 'none', color: '#ef4444', fontSize: '0.8rem', cursor: 'pointer', fontWeight: 600 }}>
              Clear all
            </button>
          </div>
        )}
        ...
        <div className="flex-col p-4 gap-3" style={{ overflowY: 'auto', flex: 1 }}>
          {notifications.length === 0 ? (
            <div className="flex-col items-center justify-center gap-3" style={{ height: '100%', color: '#475569', marginTop: '4rem' }}>
              <BellRing size={48} color="#1e293b" />
              <p>You have no notifications.</p>
            </div>
          ) : (
            notifications.map(n => (
              <div key={n.id} className="flex items-start gap-3 p-3" style={{ background: n.read ? 'rgba(255,255,255,0.02)' : 'rgba(59,130,246,0.1)', border: '1px solid', borderColor: n.read ? 'rgba(255,255,255,0.05)' : 'rgba(59,130,246,0.2)', borderRadius: '12px', transition: 'background 0.2s', position: 'relative' }} onClick={() => !n.read && markRead(n.id)}>
                <div style={{ marginTop: '2px' }}>
                  {n.type === 'alert' ? <AlertTriangleIcon /> : n.type === 'success' ? <CheckCircle size={18} color="#4ade80" /> : <Info size={18} color="#3b82f6" />}
                </div>
                <div style={{ flex: 1, paddingRight: '20px' }}>
                  <div style={{ fontSize: '0.85rem', color: n.read ? '#cbd5e1' : '#f8fafc', fontWeight: n.read ? 400 : 600, lineHeight: 1.4 }}>
                    {n.text}
                  </div>
                ...
                <button onClick={(e) => { e.stopPropagation(); deleteNotif(n.id); }} style={{ position: 'absolute', top: '12px', right: '12px', background: 'none', border: 'none', cursor: 'pointer', color: '#64748b' }}>
                  <Trash2 size={16} />
                </button>
              </div>
            ))
