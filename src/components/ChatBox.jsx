...
export default function ChatBox({ onBack }) {
  ...
  return (
    <div className="flex-col" style={{ flex: 1 }}>
      {/* Header */}
      <div className="flex items-center gap-3 p-4 border-b-dark" style={{ borderColor: 'var(--border)' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '0.72rem', color: 'var(--text-dim)' }}>Chat with HR, workers & job finders</div>
        </div>
        <button onClick={() => setShowNew(true)} style={{ background: 'rgba(225,29,72,0.1)', border: '1px solid rgba(225,29,72,0.3)', borderRadius: '10px', padding: '6px 12px', color: 'var(--primary)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.8rem', fontWeight: 600 }}>
          <Plus size={14} /> New
        </button>
      </div>
      ...
      <div className="p-4 pb-2">
        <div className="flex items-center gap-2" style={{ background: 'var(--bg-dark)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '0.6rem 1rem' }}>
          <Search size={16} color="var(--text-dim)" />
          <input placeholder="Search conversations..." style={{ background: 'transparent', border: 'none', color: 'var(--text-main)', outline: 'none', flex: 1, fontSize: '0.875rem' }} />
        </div>
      </div>
...
function RoomCard({ room, onClick }) {
  return (
    <div onClick={onClick} style={{
      ...
      padding: '0.875rem 1rem', background: 'var(--bg-dark)',
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-main)', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>{room.name}</div>
        <div style={{ fontSize: '0.75rem', color: 'var(--text-dim)', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>{room.desc}</div>
      </div>
  ...
  return (
    <div className="flex-col" style={{ flex: 1, height: '100%' }}>
      ...
      <div className="flex items-center gap-3 p-4 border-b-dark" style={{ borderColor: 'var(--border)', position: 'sticky', top: 0, background: 'var(--bg-dark)', zIndex: 10 }}>
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--text-main)' }}>{room.name}</div>
          <div style={{ fontSize: '0.7rem', color: 'var(--text-dim)' }}>{room.desc}</div>
        </div>
      </div>
        ...
        {messages.map(msg => {
          ...
          return (
            <div key={msg.id} style={{ display: 'flex', flexDirection: isMe ? 'row-reverse' : 'row', gap: '8px', alignItems: 'flex-end' }}>
              {!isMe && (
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '0.75rem', color: '#fff', flexShrink: 0 }}>
              ...
              <div style={{ maxWidth: '72%' }}>
                {!isMe && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                    <span style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--text-main)' }}>{msg.senderName}</span>
                    <span style={{ fontSize: '0.62rem', padding: '1px 6px', borderRadius: '9999px', background: 'var(--border)', color: roleColor(msg.senderRole) }}>{roleLabel(msg.senderRole)}</span>
                  </div>
                )}
                <div style={{
                  padding: '0.6rem 0.9rem', borderRadius: isMe ? '16px 16px 4px 16px' : '16px 16px 16px 4px',
                  background: isMe ? 'var(--primary)' : 'var(--bg-glass)',
      ...
      {/* Input */}
      <div className="flex items-center gap-2 p-3" style={{ position: 'absolute', bottom: 80, left: 0, right: 0, background: 'var(--bg-panel)', backdropFilter: 'blur(12px)', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <input
          value={text}
          onChange={e => setText(e.target.value)}
          onKeyDown={handleKey}
          placeholder="Type a message..."
          style={{ flex: 1, background: 'var(--bg-glass)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '0.7rem 1rem', color: 'var(--text-main)', fontSize: '0.875rem', outline: 'none' }}
        />
        <button onClick={sendMessage} disabled={!text.trim() || sending} style={{
          width: 42, height: 42, borderRadius: '12px', background: text.trim() ? 'var(--primary)' : 'var(--border)',
